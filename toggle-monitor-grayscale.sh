...
EOF
		$compositor $* -b --backend glx --window-shader-fg ${tmpfile} 2> /dev/null
			...
			shader='uniform sampler2D tex; uniform float opacity; void main() { vec4 c = texture2D(tex, gl_TexCoord[0].xy); float y = dot(c.rgb, vec3(0.4126, 0.5152, 0.1252)); gl_FragColor = opacity*vec4(y, y, y, c.a); }'
